...
enum {
  TYPE_I, TYPE_U, TYPE_S, TYPE_J, TYPE_B, TYPE_R, TYPE_N // none
};
...
static void decode_operand(Decode *s, int *dest, int *zimm, int *csr, word_t *src1, word_t *src2, word_t *imm, int type) {
...
static int decode_exec(Decode *s) {
  int dest = 0, csr = 0, zimm = 0;
  word_t src1 = 0, src2 = 0, imm = 0;
  s->dnpc = s->snpc;

#define INSTPAT_INST(s) ((s)->isa.inst.val)
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  decode_operand(s, &dest, &zimm, &csr, &src1, &src2, &imm, concat(TYPE_, type)); \
  __VA_ARGS__ ; \
}
  ...
  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = RAISEINTR(s->pc, R(17))); // R(17) is $a7
  
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, word_t t = C(csr); C(csr) = src1; R(dest) = t);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, word_t t = C(csr); C(csr) = t | src1; R(dest) = t);
  INSTPAT("??????? ????? ????? 011 ????? 11100 11", csrrc  , I, word_t t = C(csr); C(csr) = t & src1; R(dest) = t);

  INSTPAT("0011000 00010 ????? 000 ????? 11100 11", mret   , N, s->dnpc = C(MEPC); (csrs(MSTATUS) & MPIE) ? (csrs(MSTATUS) |= MIE) : (csrs(MSTATUS) &= ~(MIE)); C(MSTATUS) |= MPIE);
